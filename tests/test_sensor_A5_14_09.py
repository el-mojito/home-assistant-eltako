...
class TestSensor_A5_14_09(unittest.TestCase):
    ...
    def test_windowhandle_sensor(self):
        ...
        self.assertEqual(ewh_handle.native_value, None)
        
        msg = Regular4BSMessage(address=b'\xFF\xFF\x00\x80', data=b'\xAA\x00\x00\x08', status=0x00)
        ewh_handle.value_changed(msg)
        self.assertEqual(ewh_handle.native_value, STATE_CLOSED)
        
        msg = Regular4BSMessage(address=b'\xFF\xFF\x00\x80', data=b'\xAA\x00\x00\x0E', status=0x00)
        ewh_handle.value_changed(msg)
        self.assertEqual(ewh_handle.native_value, STATE_OPEN)
        
        msg = Regular4BSMessage(address=b'\xFF\xFF\x00\x80', data=b'\xAA\x00\x00\x0A', status=0x00)
        ewh_handle.value_changed(msg)
    ...
    def test_battery_voltage_sensor(self):
        ...
        ebs_vlt.value_changed(msg)
        self.assertAlmostEqual(ebs_vlt.native_value, 3.4, delta=1e-6)
