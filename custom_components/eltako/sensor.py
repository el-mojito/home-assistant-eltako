        ...
        for entity_config in config[platform]:
            try:
                ...
                if dev_conf.eep in [A5_13_01]:
                    ...
                    entities.append(EltakoWindowHandleWithSupplyVoltage(platform, gateway, dev_conf.id, dev_name, dev_conf.eep, SENSOR_DESC_WINDOWHANDLE))
...
class EltakoWindowHandleWithSupplyVoltage(EltakoSensor):
    """Representation of an Eltako window handle device.
    ...
    def value_changed(self, msg: ESP2Message):
        ...
        if decoded.handle_position == WindowHandleWithSupplyVoltagePosition.CLOSED:
            self._attr_native_value = STATE_CLOSED
        elif decoded.handle_position == WindowHandleWithSupplyVoltagePosition.OPEN:
            self._attr_native_value = STATE_OPEN
        elif decoded.handle_position == WindowHandleWithSupplyVoltagePosition.TILT:
            self._attr_native_value = "tilt"
        else:
            return
